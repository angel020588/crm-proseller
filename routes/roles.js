  ...
  try {
    ...
    if (usersCount > 0) {
      return res.status(400).json({
        message: `No se puede eliminar el rol. ${usersCount} usuario(s) lo tienen asignado`,
      });
    }
 ...
 * Obtener lista de permisos disponibles (solo admin)
 */
...
router.post("/assign", auth, checkRole(["admin"]), async (req, res) => {
  try {
    ...
    if (!userId || !roleId) {
      return res
        .status(400)
        .json({ message: "userId y roleId son requeridos" });
    }
