...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    const followupCount = await Followup.count({ where: { assignedTo: req.user.id, status: 'pendiente' } });
...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const [leadCount, clientCount, quotationCount, followupCount] = await Promise.all([
...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      activeDeals: leads.filter(lead => ['contactado', 'cotizado'].includes(lead.status)).length,
      closedDeals: leads.filter(lead => lead.status === 'cerrado').length,
      apiKeysActive: apiKeys.filter(key => key.isActive).length,
      followupsPending: followups.filter(f => f.status === 'pendiente').length
    };

    // Calcular tasa de conversión
    stats.conversionRate = stats.totalLeads > 0 ? 
      Math.round((stats.closedDeals / stats.totalLeads) * 100) : 0;

    // Leads de esta semana
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    stats.recentLeads = leads.filter(lead => 
...
function generateConversionTrend(leads) {
  const months = {};
  leads.forEach(lead => {
    const month = new Date(lead.createdAt).toLocaleString('es', { month: 'short' });
    if (!months[month]) {
  ...
  return Object.entries(months).map(([month, data]) => ({
    month,
    conversion: data.total > 0 ? Math.round((data.converted / data.total) * 100) : 0
  }));
}
