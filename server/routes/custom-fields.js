...
router.get('/', auth, async (req, res) => {
  try {
    const customFields = [
      { id: 1, name: 'Presupuesto', type: 'number', required: false },
      { id: 2, name: 'Industria', type: 'select', options: ['Tecnología', 'Retail', 'Servicios'] },
      { id: 3, name: 'Notas especiales', type: 'textarea', required: false }
    ];
    
    res.json({ success: true, fields: customFields });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
